...
//import com.perfect8.shop.enums.TransactionType; Version 2.0
 ...
 * Version 2.0 - Advanced inventory tracking
 */
@Repository
public interface InventoryTransactionRepository extends JpaRepository<InventoryTransaction, Long> {
    ...
    List<InventoryTransaction> findByTransactionTypeOrderByTransactionDateDesc(@Param("transactionType") String transactionType);

    /**
     * Find transactions by type with pagination
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.transactionType = :transactionType ORDER BY it.transactionDate DESC")
    Page<InventoryTransaction> findByTransactionTypeOrderByTransactionDateDesc(@Param("transactionType") String transactionType, Pageable pageable);

    /**
     * Find transactions by product and type
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.transactionType = :transactionType ORDER BY it.transactionDate DESC")
     ...
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.transactionDate BETWEEN :startDate AND :endDate ORDER BY it.transactionDate DESC")
     ...
     */
    @Query("SELECT COALESCE(SUM(it.quantityChange), 0) FROM InventoryTransaction it WHERE it.product.productId = :productId")
     ...
     */
    @Query("SELECT COALESCE(SUM(it.quantityChange), 0) FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.transactionDate BETWEEN :startDate AND :endDate")
     ...
     */
    @Query("SELECT COALESCE(SUM(it.quantityChange), 0) FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.transactionType = :transactionType")
     ...
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.referenceId = :referenceId ORDER BY it.transactionDate DESC")
    List<InventoryTransaction> findByReferenceIdOrderByTransactionDateDesc(@Param("referenceId") String referenceId);
     ...
     */
    @Query("SELECT COUNT(it) FROM InventoryTransaction it WHERE it.product.productId = :productId")
     ...
     */
    @Query("SELECT it.product.productId, COUNT(it) as transactionCount FROM InventoryTransaction it WHERE it.transactionDate >= :sinceDate GROUP BY it.product.productId HAVING COUNT(it) < :minTransactions ORDER BY transactionCount ASC")
     ...
     */
    @Query("SELECT it.product.productId, COUNT(it) as transactionCount, SUM(ABS(it.quantityChange)) as totalQuantityMoved FROM InventoryTransaction it WHERE it.transactionDate >= :sinceDate GROUP BY it.product.productId ORDER BY totalQuantityMoved DESC")
     ...
     */
    @Query("SELECT COALESCE(SUM(it.quantityChange), 0) FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.transactionDate <= :atDate")
     ...
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.quantityAfter < 0 ORDER BY it.transactionDate DESC")
     ...
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE it.product.productId = :productId AND it.quantityAfter < 0 ORDER BY it.transactionDate DESC")
     ...
     */
    @Query("SELECT it FROM InventoryTransaction it WHERE EXISTS (SELECT it2 FROM InventoryTransaction it2 WHERE it2.inventoryTransactionId != it.inventoryTransactionId AND it2.product.productId = it.product.productId AND it2.quantityChange = it.quantityChange AND ABS(TIMESTAMPDIFF(MINUTE, it2.transactionDate, it.transactionDate)) <= :minutesMargin) ORDER BY it.transactionDate DESC")
